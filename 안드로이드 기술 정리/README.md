...
앱이 실행되면 안드로이드 시스템은 **하나의 실행 스레드로** 앱의 **리눅스 프로세스를 시작**한다. 앱은 여러 구성 요소들로 구성되어 있다. 기본적으로 앱의 모든 구성 요소는 같은 프로세스와 스레드에서 실행된다. 하지만 앱 내의 여러 구성 요소가 각자 별도의 프로세스에서 실행되도록 할 수 있고, 각 프로레스에서 추가 스레드를 만들 수 있다.<br/><br/>
